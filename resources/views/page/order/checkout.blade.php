@extends('page.layout')
@section('headmeta')
<title>DangQuangTech | Thanh toán</title>
                ...
                <div class="entry" >
                  ...
                  <div class="entry-content" >
                    <h4 class="entry-title"><a href="shop-single.html">{{$cart->name}}</a></h4><span class="entry-meta">{{$cart->qty." x ".$cart->price." VNĐ"}}</span>
                     <p><span style="color:black">Bảo Hành:</span><span style="display: inline-block;background-color:{{$cart->options->color}}; width:10px;height:10px;"></span><span style="color:black">  Kích cỡ:</span><span style="color:black;font-weight:bold;">{{$cart->options->size}}</span></p>
                  </div>
                </div>
                @endforeach
              {{--   </div> --}}
