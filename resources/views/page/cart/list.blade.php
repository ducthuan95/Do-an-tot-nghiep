@extends('page.layout')
@section('headmeta')
<title>DangQuangTech | Giỏ hàng</title>
                ...
                <td>
                  <div class="product-item"><a class="product-thumb" href="{{ route('detailproduct',['alias'=>$cart->options->alias,'id'=>$cart->id]) }}"><img src="public/uploads/product/{{$cart->options->image}}" alt="{{$cart->name}}"></a>
                    <div class="product-info">
                      <h4 class="product-title"><a href="{{ route('detailproduct',['alias'=>$cart->options->alias,'id'=>$cart->id]) }}">{{$cart->name}}</a></h4><span><em>Thương Hiệu:</em> {{$cart->options->size}}</span><span><em>Bảo Hành:</em><span style="display: inline-block;background-color:{{$cart->options->color}}; width:10px;height:10px;"></span>
                    </div>
                  </div>
                </td>
